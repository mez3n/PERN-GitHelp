...
const isvolunteerMiddleware = async (req, res, next) => {
  ...
  const volunteer = await pool.query("select * from volunteer where uid $1", [
    id,
  ]);
  if (volunteer.length != 0) {
    ...
    return res
      .status(403)
      .json({ error: "Forbidden. You are not a volunteer." });
  }
};
const isPatientMiddleware = async (req, res, next) => {
  ...
  id = req.body.user.uid;
  const patient = await pool.query("select * from patient where uid $1", [id]);
  if (patient.length != 0) {
    next();
  } else {
...
const isOrganizationMiddleware = async (req, res, next) => {
  ...
  if (organization.length != 0) {
    ...
    return res
      .status(403)
      .json({ error: "Forbidden. You are not an organization." });
  }
};
const isVolunteerOrPatientMiddleware = async (req, res, next) => {
  ...
  if (volunteer.length != 0) {
    ...
    const patient = await pool.query("select * from patient where uid $1", [
      id,
    ]);
    if (patient.length != 0) {
      next();
      ...
      return res
        .status(403)
        .json({ error: "Forbidden. You are not a patient or a volunteer." });
    }
  }
};
module.exports = {
