...
const organizationController = {
  ...
  getunapprovedOrganizations: async (req, res) => {
    try {
      const organizations = await Organization.getunapprovedOrganizations();
      res.json({ organizations });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: "Internal Server Error" });
  ...
  getOrganizationLocation: async (req, res) => {
    const organizationId = req.params.id;
    try {
      const location = await Organization.getOrganizationLocation(
        organizationId
      );
      res.json({ location });
    } catch (error) {
