...
--------------------------------procedures-------------------------------------------------------
CREATE
OR REPLACE FUNCTION get_user_by_id(p_user_id INT) RETURNS TABLE (
  user_id INT,
...
VALUES
  (p_content, p_event_owner_id, p_image_id);
...
-------------------------------------------------------------------------------------------------
CREATE
OR REPLACE FUNCTION get_group_members(p_group_id INT) RETURNS TABLE (
  user_id INT,
  user_name VARCHAR(50),
  name TEXT
) AS $$ BEGIN RETURN QUERY
...
FROM
  Users U
  JOIN group_user GU ON U.uid = GU.uid
WHERE
  GU.GID = p_group_id;
