...
class Order {
  ...
  static async updateOrderState(orderid, oid, updatedOrder) {
    const result = await pool.query(
      "UPDATE orders SET state = $1 WHERE orderid = $2 AND oid = $3 RETURNING *",
      [updatedOrder.state, orderid, oid]
    );

    return result.rows[0];
  }
  static async setPostman(orderid, oid, postman_id) {
    const result = await pool.query(
      "SELECT assign_postman_to_order($1, $2, $3) AS result;",
      [orderid, postman_id, oid]
    );

    return result.rows[0];
  }
  static async updateOrderPostman(orderid, oid, updatedOrder) {
    const result = await pool.query(
      "UPDATE orders SET postman_id = $1 WHERE orderid = $2 AND oid = $3 RETURNING *",
      [updatedOrder.postman_id, orderid, oid]
    );
    return result.rows[0];
  }
